...
def augment_training_data(
    ...
    if method == "smote":
        return apply_smote(X_train, y_train, meta_train, cfg)
    elif method == "copula":
...
def _balance_participant_positives(
    X_train: pd.DataFrame,
    y_train: pd.Series,
    meta_train: pd.DataFrame,
    seed: int,
    ...
    """
    Reduce over-represented participants in the positive class before SMOTE.

    Computes the median number of positives across participants and
    subsamples any participant that exceeds 3× that median, so that no
    single participant dominates the synthetic data generated by SMOTE.
    """
    pos_mask = y_train == 1
    pos_per_pid = (
        meta_train.loc[pos_mask.values, "participant_id"]
        .value_counts()
    )

    if len(pos_per_pid) == 0:
        return X_train.copy(), y_train.copy()

    median_pos = int(np.median(pos_per_pid.values))
    cap = max(median_pos * 3, 3)  # never cap below 3 positives

    keep_idx = list(y_train[~pos_mask].index)  # all negatives kept
    rng = np.random.RandomState(seed)

    for pid, count in pos_per_pid.items():
        pid_pos_idx = meta_train[
            (meta_train["participant_id"] == pid) & pos_mask.values
        ].index.tolist()
        if count > cap:
            sampled = rng.choice(pid_pos_idx, size=cap, replace=False).tolist()
            logger.info(
                "Balance: participant %s positives reduced %d → %d",
                pid, count, cap,
            )
        else:
            sampled = pid_pos_idx
    ...
    return X_train.loc[keep_idx].reset_index(drop=True), \
           y_train.loc[keep_idx].reset_index(drop=True)
...
def apply_smote(
    ...
    First corrects origin bias by capping over-represented participants
    in the positive class, then applies SMOTE on the balanced pool.

    Parameters
    ----------
    X_train : DataFrame of shape (n_train, n_features)
    y_train : Series of shape (n_train,) — binary 0/1
    meta_train : DataFrame with participant_id column
    ...
    # Step 1: correct origin bias before SMOTE
    X_train, y_train = _balance_participant_positives(
        X_train, y_train, meta_train, cfg.seed
    )
    ...
    # On Windows, BLAS thread pools can deadlock inside NearestNeighbors.kneighbors().
    # threadpoolctl limits BLAS threads to 1 only during this call.
    try:
