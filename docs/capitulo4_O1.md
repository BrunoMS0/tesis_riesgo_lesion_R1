...
### 4.4 Dataset complementario: Runner Dataset (Löwdal et al., 2021)

Como extensión del trabajo de construcción de datasets, y con el propósito de superar la limitación de tamaño muestral del dataset PMData (16 atletas, 9 con lesiones), se incorporó como dataset de entrenamiento primario para el modelo de predicción de lesión el **Runner Dataset** publicado por Löwdal et al. (2021), disponible públicamente en el repositorio asociado a su trabajo.

**Descripción del dataset.** El Runner Dataset comprende datos longitudinales de **74 corredores** seguidos durante períodos de entre 7 meses y 2 años, con **583 eventos de lesión documentados** y una prevalencia del 1.36% (583 eventos positivos de 42,766 observaciones totales). Los datos se presentan en formato tabular de 73 columnas mediante un esquema de *7-day window wide format*: cada fila representa el resumen de 7 días anteriores de entrenamiento y bienestar para un atleta en una fecha dada. Las 10 features base disponibles son: `km` (distancia total diaria), `kmSprint` (kilómetros de sprint), `perceivedExertion`, `perceivedRecovery`, `perceivedSuccess`, `numberStrength` (sesiones de fuerza), `altInKm`, `numberOfSessions`, y dos variables derivadas de la intensidad. Cada feature tiene 7 columnas representando los días D-7 a D-1 (sufijos vacío, `.1`, `.2`, ..., `.6`). La variable objetivo `injury` es ya **prospectiva** (lesión que ocurrirá en los próximos días), por lo que no requiere desplazamiento temporal adicional.
...
| `acwr` | `acute_load_7d / (chronic_load_28d / 4)`, clipado a 5.0 | `acwr` |
| `high_intensity_km_7d` | Suma de km de sprint 7 días | `trimp_7d_sum` (proxy) |
...
| `alt_hours_7d` | Altitud acumulada 7 días | — |
