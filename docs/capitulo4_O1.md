...
Análisis estadístico. Se llevaron a cabo cuatro análisis estadísticos complementarios para fundamentar la selección de variables. El análisis se realizó sobre el dataset exploratorio en su estado previo a la etapa de filtrado de nulidad, el cual contenía **56 variables numéricas**: estas corresponden a las variables originales provenientes de las 10 fuentes Fitbit y las 3 fuentes PMSYS (incluyendo columnas que posteriormente serían eliminadas por superar el umbral del 60% de valores nulos), más las variables derivadas calculadas en la fase de ingeniería de características. Tras aplicar el filtro de columnas con alta nulidad (>60%) en el pipeline ETL (R2) y el criterio de consenso triple de selección, el dataset final (R3) quedó reducido a **33 variables predictoras**, contenidas en un total de 49 columnas que incluyen los identificadores de participante y fecha, y la variable objetivo "is_injured". Esta diferencia entre los 56 variables analizadas estadísticamente y las 49 columnas del dataset final refleja el proceso de depuración sistemática descrito en la Sección 4.2.2.

En primer lugar, se realizaron tests de normalidad de Shapiro-Wilk sobre las 56 variables numéricas del dataset exploratorio. Los resultados evidenciaron que la totalidad de las variables (56 de 56) rechazaron la hipótesis nula de normalidad (p < 0.05). Este hallazgo fundamentó la elección de métodos no paramétricos para los análisis subsiguientes. Las estadísticas descriptivas completas y los resultados de los tests de normalidad se encuentran en el Anexo D.
...
En cuarto lugar, se realizó un test de Kruskal-Wallis para comparar las distribuciones de cada variable entre los grupos lesionado y no lesionado, identificando las variables con mayor capacidad discriminante.

Adicionalmente, se realizó un Análisis de Componentes Principales (PCA) con tres propósitos concretos: (1) **validar que las variables derivadas capturan dimensiones fisiológicas genuinamente distintas** y no redundantes entre sí, es decir, que la ingeniería de características produce información nueva que ninguna variable original recoge por sí sola; (2) **diagnosticar el grado de redundancia del espacio de features**, evaluando cuánta de la varianza total puede ser explicada con pocos componentes; y (3) **confirmar la coherencia teórica del conjunto de variables**, verificando que los componentes principales se corresponden con las dimensiones fisiológicas que la literatura identifica como determinantes del riesgo de lesión. Los resultados muestran que los tres primeros componentes capturan el 45.22% de la varianza total, con el PC1 dominado por métricas de carga de entrenamiento (ACWR, cargas aguda y crónica), el PC2 por variables de sueño y recuperación (minutesAsleep, sleep_debt, overall_score) y el PC3 por métricas de actividad física (steps, active_ratio). Este patrón confirma que el espacio de features captura al menos tres dimensiones fisiológicas independientes — carga, recuperación y actividad — coherentes con el marco teórico de la tesis sobre los determinantes del riesgo de lesión. La varianza restante (54.78%) distribuida en los demás componentes indica que el dataset es suficientemente rico para alimentar modelos multivariados. Los resultados completos de los análisis de correlación, multicolinealidad y PCA se encuentran en el Anexo E.
